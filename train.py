...
import loader_imagenet_like as imagenet_like
...
parser = argparse.ArgumentParser(
    description='PyTorch Semi-supervised learning Training')
...
parser.add_argument('--dataset', '-d', metavar='DATASET', default='cifar10_zca',
                    help='dataset: '+' (default: cifar10)', choices=['cifar10', 'cifar10_zca', 'svhn', 'openimages'])
...
parser.add_argument('--boundary', default=0, type=int,
                    help='different label/unlabel division [0,9]')
parser.add_argument('--gpu', default=0, type=str, help='cuda_visible_devices')
parser.add_argument('--root', default="", type=str, help='dataset root path')
...
def main():
    ...
    if args.arch == 'preresnet':
        ...
        model = wideresnet.WideResNet(
            28, args.num_classes, widen_factor=widen_factor, dropRate=drop_rate, leakyRate=0.1)
    else:
        assert(False)
    ...
    ckpt_dir = args.ckpt+'_'+args.dataset+'_' + \
        args.arch+'_'+args.model+'_'+args.optim
    ...
    if args.dataset == 'cifar10':
        ...
        dataloader = imagenet_like.ImagenetLike
        normalize = transforms.Normalize(mean=[0.4914, 0.4822, 0.4465],
                                         std=[0.2023, 0.1994, 0.2010])
    ...
    labelset = dataloader(root=data_dir, split='label', download=True,
                          transform=transform_train, boundary=args.boundary)
    ...
    validset = dataloader(root=data_dir, split='valid', download=True,
                          transform=transform_test, boundary=args.boundary)
    ...
    testset = dataloader(root=data_dir, split='test',
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.model == 'baseline':
            ...
            for i in range(10):  # baseline repeat 10 times since small number of training set
                prec1_tr, loss_tr = train_sup(
                    label_loader, model, criterions, optimizer, epoch, args)
                weight_cl = 0.0
        elif args.model == 'pi':
            print('Pi model')
            prec1_tr, loss_tr, loss_cl_tr, weight_cl = train_pi(
                label_loader, unlabel_loader, model, criterions, optimizer, epoch, args)
        elif args.model == 'mt':
            print('Mean Teacher model')
            prec1_tr, loss_tr, loss_cl_tr, prec1_t_tr, weight_cl = train_mt(
                label_loader, unlabel_loader, model, model_teacher, criterions, optimizer, epoch, args)
        else:
        ...
        prec1_val, loss_val = validate(
            val_loader, model, criterions, args, 'valid')
        prec1_test, loss_test = validate(
            test_loader, model, criterions, args, 'test')
        if args.model == 'mt':
            prec1_t_val, loss_t_val = validate(
                val_loader, model_teacher, criterions, args, 'valid')
            prec1_t_test, loss_t_test = validate(
                test_loader, model_teacher, criterions, args, 'test')
        ...
        save_checkpoint(dict_checkpoint, is_best,
                        args.arch.lower()+str(args.boundary), dirname=ckpt_dir)
